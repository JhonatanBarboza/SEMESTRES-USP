...
#define minx -48     // Valor mínimo do domínio da função em X
#define maxy 75      // Valor máximo do domínio da função em Y
#define miny -75     // Valor mínimo do domínio da função em Y
...
float melhor_media(float maxfit, float media, int tampop){
...
void elitismo(int tampop) {
    int maxi = 1, mimi = 1;
    float media = 0.0f;
    float maxfit = fit[1], mimfit = fit[1];
    ...
    media = melhor_media(maxfit, media, tampop);
    ...
    for (int i = 1; i <= tampop; i++) {
        ...
        if (i == mimi) continue;  // Protege o melhor indivíduo
