...
int gen = 1;            // Geração
int numger = 20;        // Número de gerações
...
void elitismo(int tampop) {
    ...
    float media = 0.0f;
    float maxfit = fit[1];
    float vetmaxi[tampop], vetmedia[20];;
    ...
    vetmaxi[gen-1] = fit[maxi]; // plotar o gráfico



    printf("o melhor individuo é : %f\n", vetmaxi[gen-1]);



    // Guarda a média da população
    for (int i = 1; i <= tampop; i++) { // loop começa em 1 se fit[1] é o primeiro elemento
        media += fit[i];
    }

    media = media / tampop;
    vetmedia[gen-1] = media; // plotar o gráfico


    printf("a media da população é : %f\n", vetmedia[gen-1]);

    printf("a media da população é2222 : %f\n", media );


     // Opcional: Imprimir para depuração
    //printf("Max fitness: %f,    Média: %f\n", vetmaxi[gen-1], media);
...
void ajustaTaxaMutacao(int tampop) {
    ...
    if (cont >= (tampop * (tampop - 1) / 2) * 0.9) {
        TaxMut = 12.0; // Aumenta a taxa de mutação
    } else {
        if (cont >= (tampop * (tampop - 1) / 2) * 0.5) {
            TaxMut = 0.00001;}//diminui a taxa de mutação para os individuos convergirem
        else {    
...
double gerarMutacao(double media, double desvioPadrao) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::normal_distribution<> dist(media, desvioPadrao);
    return dist(gen);
}

void aplicarMutacao(double* ind, int i, double TaxMut, double minx, double maxx) {
    double mutacao = gerarMutacao(0, TaxMut); // media 0, desvio padrão TaxMut
    ind[i] += mutacao;
    ind[i] = std::max(minx, std::min(maxx, ind[i]));  // Garantir que permaneça no domínio
}
*/